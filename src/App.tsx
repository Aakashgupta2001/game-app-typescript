...
function App() {
  ...
  const incorrectLetters = lettersGuessed.filter((letter) => !wordGuessed.includes(letter));
  const isLoser = incorrectLetters.length >= 6;
  const isWinner = wordGuessed.split("").every((letter) => lettersGuessed.includes(letter));

  const addLettersGuessed = React.useCallback(
    (letter: string) => {
      if (lettersGuessed.includes(letter) || isLoser || isWinner) return;

      setLettersGuessed((currentLetters) => [...currentLetters, letter]);
    },
  ...
  return (
    <div className="App">
      <div className="response">
        ...
        {isLoser && <button onClick={() => window.location.reload()}>Restart</button>}
        {isWinner && <button onClick={() => window.location.reload()}>Restart</button>}
      </div>
